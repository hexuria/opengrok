...
export const SAND_DEEP_LINK_SCHEME = "sand"; export const OPENGROK_DEEP_LINK_SCHEME = "opengrok"; /** The V2 side-by-side build's own scheme; parsed everywhere, declared only by V2. */ export const OPENGROK_V2_DEEP_LINK_SCHEME = "opengrokv2"; export const SAND_DEEP_LINK_AUTHORITY = "app"; export const SAND_HTTPS_DEEP_LINK_ORIGIN = "https://cursor.com"; export const SAND_HTTPS_DEEP_LINK_PATH_PREFIX = "/sand/link"; export const SAND_DEEP_LINK_MAX_LENGTH = 2_048;
...
export function parseSandDeepLink(raw: unknown): ParsedSandDeepLink | null {
  ...
  const lower = raw.toLowerCase(); const source = lower.startsWith(`${SAND_DEEP_LINK_SCHEME}:`) || lower.startsWith(`${OPENGROK_DEEP_LINK_SCHEME}:`) || lower.startsWith(`${OPENGROK_V2_DEEP_LINK_SCHEME}:`) ? "protocol" : lower.startsWith("https:") ? "https" : null; if (source == null) return null;
  let url: URL; try { url = new URL(raw); } catch { return null; } if (url.username !== "" || url.password !== "" || url.port !== "") return null;
  if (source === "protocol") { if ((url.protocol !== `${SAND_DEEP_LINK_SCHEME}:` && url.protocol !== `${OPENGROK_DEEP_LINK_SCHEME}:` && url.protocol !== `${OPENGROK_V2_DEEP_LINK_SCHEME}:`) || url.host !== SAND_DEEP_LINK_AUTHORITY) return null; if (url.pathname === CUSTOM_MESSAGE_ROUTE_PATH) return parseMessageLink(url, source); if (url.pathname === CUSTOM_COLLECTION_ROUTE_PATH) return parseCollectionLink(url); if (url.pathname === SAND_PLUGIN_DEEP_LINK_PATH) return parsePluginAddLink(url, source); if (url.pathname === CUSTOM_OPEN_ROUTE_PATH) return parseOpenLink(url, source); if (url.pathname !== CUSTOM_INFO_ROUTE_PATH) return null; }
