...
test("packaging does not copy Anysphere Mach-Os into dist/native", async () => {
  ...
  assert.doesNotMatch(asar, /path\.join\(runtimeUnpacked, "deps"\)/);
  ...
  assert.match(audit, /walkIfPresent\(path\.join\(repoRoot, depsRoot\)\)/);
  assert.match(audit, /status: "artifact-runtime-boundary"/);
