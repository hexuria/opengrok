...
whenFrontend("React ports are wired: Computer/Dictation/Usage, panes, rail, hosts", async () => {
  ...
  assert.match(view, /\.sand-message \{[^}]*min-width: 0;/);
  assert.match(view, /\.sand-message \{[^}]*overflow-wrap: anywhere;/);
  assert.match(view, /\.sand-message \{[^}]*max-width: min\(88%, 640px, calc\(100% - 82px\)\);/);
  ...
  assert.match(view, /\.sand-message-action-anchor \{[^}]*min-width: 0;/);
  assert.match(view, /\.sand-message-hover-actions \{[^}]*transform: translateY\(-100%\);/s);
