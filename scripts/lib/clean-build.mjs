...
export const runtimeComposition = Object.freeze([
  ...
  { runtime: "electron-runtime-dependencies", path: "dist/deps", mode: "generated-runtime", reason: "Public native addons (better-sqlite3, tree-sitter, tree-sitter-bash) are rebuilt against Electron 42.1.0 (ABI 146) headers and staged as dist/deps; whichlang-node and @anysphere/tree-chunk-napi are refused when a production esbuild metafile mentions them rather than copied from 0.18; cursor-proclist is omitted and process scan no-ops." },
  ...
  { runtime: "electron-shell", path: "Contents/Frameworks/Electron Framework.framework", mode: "artifact-runtime", reason: "The macOS package wraps npm Electron 42.1.0 (ABI-matched to the 0.18 runtime)." },
]);
...
async function buildRuntimeDistribution({ outputRoot, composition }) {
  ...
  const renderer = await buildProductionRenderer({ outputRoot });
  ...
  const buildManifest = {
    ...
    buildKind: "source-aware-reconstruction",
    deterministicInputs: [
      "source",
      "frontend/src",
...
export async function buildCleanDistribution({ outputRoot = cleanBuildDir } = {}) {
  return buildRuntimeDistribution({ outputRoot, composition: runtimeComposition });
}

export async function overlayCleanDistribution(outputRoot, { stageRoot = stagedAppDir, composition = runtimeComposition } = {}) {
  for (const relative of executableReplacements) {
