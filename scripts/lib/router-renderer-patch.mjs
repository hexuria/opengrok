...
const REGISTRY_AFTER = 'const wDn=[{id:"general",label:"General",icon:"settings-gear"},{id:"router",label:"Router",icon:"git-branch"},{id:"dictation",label:"Dictation",icon:"mic"},{id:"usage",label:"Usage",icon:"chart-bars"},{id:"beta",label:"Updates",icon:"cloud-download"}]';
...
let RBoxLast=null;
function ROpenGrokActive(){const[v,setV]=de.useState(()=>(RBoxLast&&RBoxLast.mode)==="opengrok");
...
function ROpenGrokServer(){const[s,e]=de.useState({url:"",token:"",saved:null,hasToken:!1,status:null,busy:!1,error:null,dirty:!1});
const load=()=>window.desktop.agent.getOpenGrokServer().then(r=>{if(r==null)return;e(i=>({...i,saved:r.gatewayUrl??null,url:i.dirty||i.busy?i.url:(r.gatewayUrl??""),hasToken:!!r.hasToken,status:r.status??null}))}).catch(()=>{});
de.useEffect(()=>{load();const id=setInterval(load,15e3);return()=>clearInterval(id)},[]);
const save=async()=>{e(i=>({...i,busy:!0,error:null}));try{const r=await window.desktop.agent.setOpenGrokServer(s.url.trim(),s.token.trim()||void 0);e(i=>({...i,busy:!1,token:"",dirty:!1,saved:r.gatewayUrl??null,hasToken:!!r.hasToken,status:r.status??null}));window.dispatchEvent(new CustomEvent("sand-opengrok-changed",{detail:{gatewayUrl:r.gatewayUrl??null}}))}catch(err){e(i=>({...i,busy:!1,error:String(err&&err.message||err)}))}};
const clear=async()=>{e(i=>({...i,busy:!0,error:null}));try{await window.desktop.agent.setOpenGrokServer("");e(i=>({...i,busy:!1,url:"",token:"",dirty:!1,saved:null,hasToken:!1}));window.dispatchEvent(new CustomEvent("sand-opengrok-changed",{detail:{gatewayUrl:null}}))}catch(err){e(i=>({...i,busy:!1,error:String(err&&err.message||err)}))}};
const st=s.status,connected=st&&st.ok,detail=s.saved==null?"No server saved yet.":(st&&st.detail)||"Not connected yet.";
return a.jsxs("div",{className:"sand-9f619 sand-78zum5 sand-dt5ytf",style:{gap:10},children:[
a.jsx(ie,{description:"Where your OpenGrok server is reachable, including the scheme and port.",label:"Server URL",variant:"card",children:a.jsx("input",{"aria-label":"OpenGrok server URL",className:RRouterInputClass,disabled:s.busy,onChange:v=>e(i=>({...i,url:v.currentTarget.value,dirty:!0})),placeholder:"http://192.168.1.10:1447",style:{fontSize:13,height:34,minWidth:0,padding:"0 10px",width:300},type:"text",value:s.url})}),
a.jsx(ie,{description:s.hasToken?"A bearer is saved. Paste a new one to replace it.":"Stored with your other Grok Bot secrets, never in settings.json.",label:"Bearer token",variant:"card",children:a.jsx("input",{"aria-label":"OpenGrok bearer token",className:RRouterInputClass,disabled:s.busy,onChange:v=>e(i=>({...i,token:v.currentTarget.value,dirty:!0})),placeholder:s.hasToken?"Replace saved token":"Paste bearer token",style:{fontSize:13,height:34,minWidth:0,padding:"0 10px",width:300},type:"password",value:s.token})}),
a.jsxs("div",{className:"sand-9f619 sand-78zum5 sand-6s0dn4 sand-h8yej3",style:{gap:8,flexWrap:"wrap"},children:[
a.jsx(oe,{disabled:s.busy,onClick:save,shape:"rectangular",size:"sm",variant:"secondary",children:s.busy?"Saving\u2026":"Save and connect"}),
s.saved?a.jsx(oe,{disabled:s.busy,onClick:clear,shape:"rectangular",size:"sm",variant:"secondary",children:"Clear"}):null,
...
function RBoxRuntime(){const all=[{value:"remote",label:"Grok VM"},{value:"local-docker",label:"Local VM"},{value:"windows365",label:"Windows 365"},{value:"opengrok",label:"OpenGrok Server"}];const[s,e]=de.useState(()=>RBoxLast??{mode:null,provider:null,status:null,error:null,busy:!1,windows365:null,account:null});de.useEffect(()=>{let t=!0;const apply=r=>{if(!t||r==null)return;e(i=>{const next={...i,mode:i.busy?i.mode:typeof r.mode==="string"?r.mode:i.mode,status:r.status??null,windows365:r.windows365??null,account:r.account??null,error:null};RBoxLast=next;return next})};const load=()=>window.desktop.agent.getBoxRuntime().then(apply).catch(r=>{t&&e(i=>({...i,error:String(r&&r.message||r)}))});window.desktop.agent.getInferenceRouter().then(r=>{if(!t||r==null)return;e(i=>{const next={...i,provider:typeof r.provider==="string"?r.provider:i.provider};RBoxLast=next;return next})}).catch(()=>{});const onProvider=n=>{const p=n&&n.detail&&n.detail.provider;if(typeof p==="string")e(i=>({...i,provider:p}))};window.addEventListener("sand-router-provider-changed",onProvider);load();const id=setInterval(load,15e3);return()=>{t=!1;clearInterval(id);window.removeEventListener("sand-router-provider-changed",onProvider)}},[]);const RBoxOptions=s.provider==="cursor"?all:all.filter(n=>n.value!=="remote"||n.value==="opengrok");const t=async n=>{if(n==null)return;if(n==="remote"&&s.provider!=="cursor")n="local-docker";e(i=>({...i,mode:n,busy:!0,error:null}));try{const r=await window.desktop.agent.setBoxRuntime(n);e(i=>({...i,mode:n,status:r&&r.status||null,windows365:r&&r.windows365||null,account:r&&r.account||null,busy:!1}));window.dispatchEvent(new CustomEvent("sand-box-runtime-changed",{detail:{mode:n}}))}catch(r){e(i=>({...i,mode:n,busy:!1,error:String(r&&r.message||r)}))}};const n=s.mode==="opengrok"?"Your OpenGrok server holds the bots and runs their work.":s.mode==="windows365"?"Enter Windows 365 credentials below, then Save and Connect.":s.mode==="remote"?(s.account&&s.account.detail||"Uses the Cursor account already signed into Grok Bot."):(s.status&&s.status.detail||"A Linux desktop in Docker on this Mac.");return a.jsxs("div",{children:[a.jsx(ie,{description:n,label:"Computer for this account",variant:"card",children:a.jsx(ye,{"aria-label":"Computer for this account",onValueChange:t,options:RBoxOptions,placement:"bottom-end",size:"lg",value:s.mode==null?null:RBoxOptions.some(r=>r.value===s.mode)?s.mode:"local-docker",variant:"filled"})}),s.mode==="windows365"?a.jsxs("div",{style:{marginTop:8},children:[a.jsx(se,{as:"p",size:"sm",children:"Windows 365 credentials"}),a.jsx(RW365Setup,{})]}):null,s.mode==="opengrok"?a.jsx("div",{style:{marginTop:8},children:a.jsx(ROpenGrokServer,{})}):null,s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:String(s.error)}):null]})}
function RRouterPanel(){const[s,e,g,u]=RRouterState(),[t,n]=RRouterSecrets(),r=RRouterProviders.find(i=>i.value===s.provider)??RRouterProviders[0],i=s.usage?.providers?.[s.provider]??RRouterEmptyUsage,o=r.value==="codex"?"Official Codex/ChatGPT login on this Mac.":r.kind==="local"?"Official Claude login on this Mac.":r.kind==="key"?"Stored with your other Grok Bot secrets.":"Uses the account already connected to Grok Bot.";return a.jsx(Te,{children:a.jsxs("div",{className:k("sand-settings-general","sand-9f619 sand-78zum5 sand-dt5ytf sand-3qzy4x"),children:[ROpenGrokActive()?a.jsx(re,{title:"Routing",children:a.jsx(ie,{description:"Your OpenGrok server runs the work on each coworker\u2019s own model, so the router provider does not apply.",label:"Provider",variant:"card",children:a.jsx(se,{as:"span",color:"primary",size:"sm",children:"OpenGrok Server"})})}):a.jsx(re,{title:"Routing",children:a.jsx(ie,{description:r.description,label:"Provider",variant:"card",children:a.jsx(ye,{"aria-label":"Routing provider",onValueChange:l=>{if(l!==null)void e(l)},options:RRouterOptions,placement:"bottom-end",size:"lg",value:s.provider,variant:"filled"})})}),a.jsx(re,{title:"Computer",children:a.jsx(RBoxRuntime,{})}),a.jsx(re,{title:"Performance",children:a.jsx(RHardwareAcceleration,{})}),a.jsx(re,{title:r.kind==="key"?"OpenRouter account":"Account",children:a.jsx(ie,{description:o,label:r.kind==="key"?"API key":"Status",variant:"card",children:a.jsx(RRouterCredential,{provider:r,state:s,keys:t,onSaved:n,onLogin:g})})}),r.kind==="key"?a.jsx(re,{title:"Model",children:a.jsx(ie,{description:"Any OpenRouter model id, including :free models.",label:"Model",variant:"card",children:a.jsx(ROpenRouterModel,{model:s.openRouterModel,onSaved:l=>u(c=>({...c,openRouterModel:l.openRouterModel??l.model??c.openRouterModel,error:null}))})})}):null,s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null]})})}
