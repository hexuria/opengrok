...
function ROpenGrokComputers(){const[s,e]=de.useState({computers:null,signedIn:!1,error:null,computerError:null,activeKind:null});
const load=()=>window.desktop.agent.listOpenGrokComputers().then(r=>{if(r==null)return;e({computers:r.computers||[],signedIn:!!r.signedIn,error:r.error||null,computerError:r.computerError||null,activeKind:r.activeKind||null})}).catch(err=>e(i=>({...i,error:String(err&&err.message||err)})));
...
if(s.computerError){const c=ROpenGrokErrorCopy[s.computerError.code]||ROpenGrokErrorCopy.unknown,detail=String(s.computerError.message||"").trim();return a.jsxs("div",{children:[a.jsx(ie,{description:c[1],label:c[0],variant:"card",children:a.jsx(se,{as:"span",color:"red",size:"sm",children:"No computer"})}),detail?a.jsx(se,{as:"p",color:"secondary",size:"sm",style:{marginTop:8},children:detail}):null,s.computers.length>0?a.jsx("div",{style:{marginTop:8},children:ROpenGrokRows(s.computers,s.activeKind)}):null]})}
...
return ROpenGrokRows(s.computers,s.activeKind)}
