...
return a.jsx("div",{children:s.computers.map((c,i)=>{const kindName=ROpenGrokKind[c.kind]||(c.kind?String(c.kind):""),ready=c.configured!==!1,detail=[kindName,c.state?String(c.state):null].filter(Boolean).join(" \u00b7 ")||"On your OpenGrok server.";return a.jsx(ie,{divided:i>0,description:detail,label:String(c.label||c.name||kindName||c.id),variant:"card",children:a.jsx(se,{as:"span",color:"secondary",size:"sm",children:ready?String(c.state||"Ready"):"Set up by your org admin"})},String(c.id))})})}
