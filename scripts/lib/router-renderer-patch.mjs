...
// "12 rules — 8 allow, 4 never", singular-aware, or an invitation when empty.
const RStandingSummary=(allow,deny)=>{const total=allow+deny;
  if(total===0)return "Commands you answer Always or Never for will be listed here.";
  return total+(total===1?" rule":" rules")+" — "+allow+" allow, "+deny+" never. Delete one to be asked again.";};
const RRuleMono={fontFamily:"ui-monospace,SFMono-Regular,Menlo,monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"};
// One rule: the command on the left, its action on the right. Inside the
...
function RHardwareAcceleration(){
  ...
  return a.jsxs("div",{children:[
    a.jsx(ie,{description:"Uses the Mac's GPU for rendering (Metal), like official Grok Bot 0.29. Turn off only if you see visual glitches.",label:"Hardware acceleration",variant:"card",children:a.jsx(Ne,{label:"Hardware acceleration",isChecked:!!t.enabled,size:"sm",onToggle:v=>{void g(v)}})}),
...
const DEEPLINK_MSG_HELPER = ';(()=>{try{'
  + 'var ring=function(){try{var c=getComputedStyle(document.body).color;var m=/rgba?\\(([^)]+)\\)/.exec(c);if(m){var p=m[1].split(",");return "rgba("+p[0].trim()+","+p[1].trim()+","+p[2].trim()+",.5)"}}catch(_){}return "rgba(150,150,158,.55)"};'
  + 'var openMessage=function(agentId,messageId,indexHint){var tries=0;var sel=function(id){var q=JSON.stringify(String(id));return "[data-entry-id="+q+"],[data-row-key="+q+"]"};var found=function(el){var n=0;var center=function(){el.scrollIntoView({block:"center"});if(++n<6)setTimeout(center,350)};center();el.style.transition="box-shadow .3s";el.style.boxShadow="0 0 0 2px "+ring();setTimeout(function(){el.style.boxShadow=""},5000)};var jump=function(idx){var sc=document.querySelector(".sand-virtual-transcript");if(!sc)return;var lo=0,hi=sc.scrollHeight,it=0;var step=function(){var el=document.querySelector(sel(messageId));if(el){found(el);return}var rows=Array.prototype.slice.call(document.querySelectorAll("[data-index]")).map(function(r){return parseInt(r.getAttribute("data-index"),10)}).filter(isFinite);if(rows.length){var mn=Math.min.apply(null,rows),mx=Math.max.apply(null,rows);if(idx<mn)hi=sc.scrollTop;else if(idx>mx)lo=sc.scrollTop;sc.scrollTop=(lo+hi)/2}if(++it<26)setTimeout(step,280);else hunt()};sc.scrollTop=sc.scrollHeight/2;setTimeout(step,450)};var openChat=function(){var row=document.querySelector(".sand-agent-item[data-agent-id="+JSON.stringify(String(agentId))+"]");if(!row){if(++tries<20)setTimeout(openChat,400);return}row.click();'
  ...
  + 'var glow=function(){var w=0;var wait=function(){var el2=document.querySelector(sel(messageId));if(!el2&&w>5){var mid=document.elementFromPoint(innerWidth/2,innerHeight/2);el2=mid&&mid.closest?mid.closest("[data-row-key]"):null}if(el2){el2.style.transition="box-shadow .3s";el2.style.boxShadow="0 0 0 2px "+ring();setTimeout(function(){el2.style.boxShadow=""},5000);return}if(++w<24)setTimeout(wait,250)};wait()};'
  ...
  + 'var steps=0;var hunt=function(){var sc=document.querySelector(".sand-virtual-transcript");if(!sc)return;var el=document.querySelector("[data-entry-id=\'"+messageId+"\']");if(el){el.scrollIntoView({block:"center"});el.style.transition="box-shadow .3s";el.style.boxShadow="0 0 0 2px "+ring();setTimeout(function(){el.style.boxShadow=""},4000);return}if(steps===0)sc.scrollTop=0;else{var before=sc.scrollTop;sc.scrollTop=Math.min(sc.scrollHeight,before+sc.clientHeight*0.95);if(sc.scrollTop>=sc.scrollHeight-sc.clientHeight-4&&steps>2)return}if(++steps<120)setTimeout(hunt,320)};openChat()};'
