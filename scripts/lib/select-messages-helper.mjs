...
// row's checkbox sits at its middle; a tall one's near its top, where the eye starts.
//
...
const CSS =
  ...
  + '+".sand-sel-box{position:absolute;width:18px;height:18px;border-radius:4px;border:2px solid rgba(127,127,127,.7);background:rgba(255,255,255,.92);box-sizing:border-box}"'
  + '+".sand-sel-box.on{background:#1d9bf0;border-color:#1d9bf0}"'
  + '+".sand-sel-box.on::after{content:\\"\\";position:absolute;left:4px;top:1px;width:5px;height:9px;border:solid #fff;border-width:0 2px 2px 0;transform:rotate(45deg)}"'
...
export const SELECT_MODE_HELPER =
  ...
  + 'var entryIdOf=function(row){if(!row||!row.getAttribute)return null;var lab=row.getAttribute("aria-labelledby")||"";var m=/sand-conversation-entry-(.+?)-(?:author|timestamp|body)(?:\\s|$)/.exec(lab);if(m)return m[1];var one=row.getAttribute("data-entry-id");if(one)return one;if(!(row.classList&&row.classList.contains("sand-transcript-row")))return null;var key=row.getAttribute("data-row-key")||"";return ADDR.test(key)||/^e_/.test(key)?key:null};'
  ...
  + 'var renderBar=function(msg){if(!st.on){bar.hidden=true;return}bar.hidden=false;bar.textContent="";placeBar();var n=st.ids.size;'
  ...
  + 'var onKey=function(ev){if(!st.on)return;if(ev.key==="Escape"){ev.preventDefault();ev.stopPropagation();api.exit()}else if((ev.metaKey||ev.ctrlKey)&&(ev.key==="a"||ev.key==="A")&&!(ev.target&&/^(INPUT|TEXTAREA)$/.test(ev.target.tagName))&&!(ev.target&&ev.target.isContentEditable)){ev.preventDefault();ev.stopPropagation();api.selectAll()}};'
  ...
  + 'enter:function(seed){if(st.on)return;st.on=true;st.ids=new Set();st.anchor=null;if(seed!=null&&String(seed).length>0)st.ids.add(String(seed));var sc=scroller();sc&&mo.observe(sc,{childList:true,subtree:true,attributes:true,attributeFilter:["style"]});iv=setInterval(queue,300);document.addEventListener("click",onClick,true);document.addEventListener("keydown",onKey,true);document.addEventListener("scroll",queue,true);window.addEventListener("resize",queue);renderBar();queue()},'
  + 'exit:function(){if(!st.on)return;st.on=false;st.ids.clear();mo.disconnect();clearInterval(iv);document.removeEventListener("click",onClick,true);document.removeEventListener("keydown",onKey,true);document.removeEventListener("scroll",queue,true);window.removeEventListener("resize",queue);bar.hidden=true;layer.hidden=true;layer.textContent=""},'
