...
// button, the count, and icon buttons (save to a collection, delete, done); every
...
const ICONS =
  'var ICON={'
  + 'stack:\'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="9" width="14" height="11" rx="2.5"/><path d="M6.5 6h11a2.5 2.5 0 0 1 2.5 2.5v9"/><path d="M9.5 3h8A3.5 3.5 0 0 1 21 6.5v8"/></svg>\','
...
// transform) move with it.
  ...
  // The toolbar: [add loaded] N selected · Clear · M loaded ……… save · delete · done
  ...
  + 'if(col&&col.addMessages){iconButton("stack","Save to a collection",function(){picker()}).disabled=!n}'
  ...
  + 'var send=function(extra,label){var ag=agentIdNow();var ids=Array.from(st.ids);var col=window.desktop&&window.desktop.collections;if(!col||!col.addMessages)return;if(!ag){renderBar("No active agent detected");setTimeout(function(){renderBar()},2500);return}if(!ids.length){renderBar("Choose at least one message first.");setTimeout(function(){renderBar()},2500);return}renderBar(label);var req={agentId:ag,entryIds:ids};for(var k in extra)req[k]=extra[k];col.addMessages(req).then(function(){renderBar("Added "+ids.length+".");setTimeout(function(){api.exit()},1200)}).catch(function(e){renderBar("Failed: "+(e&&e.message||e));setTimeout(function(){renderBar()},3000)})};'
  + 'var picker=function(){var col=window.desktop&&window.desktop.collections;if(!col||!col.list){send({},"Saving\\u2026");return}bar.textContent="";placeBar();note("Add "+st.ids.size+" to\\u2026","sand-sel-count");'
  + 'col.list().then(function(r){var cols=(r&&r.collections)||[];cols.slice(0,6).forEach(function(c){textButton(c.name+" ("+(c.count||0)+")",function(){send({collectionId:c.id},"Saving\\u2026")})});'
  + 'textButton("New collection\\u2026",function(){bar.textContent="";placeBar();note("Name:","sand-sel-count");var inp=document.createElement("input");inp.type="text";inp.placeholder="Collection name";inp.addEventListener("keydown",function(ev){ev.stopPropagation();if(ev.key==="Enter"&&inp.value.trim())send({name:inp.value.trim()},"Saving\\u2026");if(ev.key==="Escape")renderBar()});bar.appendChild(inp);textButton("Create",function(){inp.value.trim()&&send({name:inp.value.trim()},"Saving\\u2026")});textButton("Back",function(){picker()});inp.focus()});'
  + 'var sp=document.createElement("span");sp.className="sand-sel-spacer";bar.appendChild(sp);iconButton("x","Back",function(){renderBar()})}).catch(function(){send({},"Saving\\u2026")})};'
