...
var rowOf=function(m){return {model:String(m.modelId||m.model||""),requests:Number(m.requests)||0,tokIn:Number(m.inputTokens)||0,tokOut:Number(m.outputTokens)||0,list:num(m.listUsd),actual:num(m.costUsd),points:m.points==null?null:Number(m.points)}};
...
var table=function(u,filter){var rows=(u&&Array.isArray(u.models)?u.models:[]).map(rowOf);if(filter&&filter!=="all")rows=rows.filter(function(r){return r.model===filter});
  var t={requests:0,tokIn:0,tokOut:0,list:0,actual:0,points:0,pointsKnown:true};rows.forEach(function(r){t.requests+=r.requests;t.tokIn+=r.tokIn;t.tokOut+=r.tokOut;t.list+=r.list||0;t.actual+=r.actual||0;if(r.points==null)t.pointsKnown=false;else t.points+=r.points});return {rows:rows,totals:t}};
...
var open=function(agentId){agentId=agentId||currentAgent();var a=window.desktop&&window.desktop.agent;if(!agentId||!a||!a.getCoworkerUsage)return null;close();style();
  ...
  var paint=function(){tbody.textContent="";var u=m.usage;if(!u)return;var tt=table(u,m.filter);
    tt.rows.forEach(function(r){var tr=el("tr");var sh=window.__sandModels&&m.cat?window.__sandModels.shown(m.cat,r.model):"";tr.append(el("td",null,r.model+sh),el("td","n",int(r.requests)),el("td","n",int(r.tokIn)+" / "+int(r.tokOut)),el("td","n",r.list==null?"—":usd(r.list)),el("td","n",r.actual==null?"—":usd(r.actual)),el("td","n",pts(r.points)));tbody.appendChild(tr)});
    var t=tt.totals;var tr=el("tr","t");tr.append(el("td",null,(m.filter==="all"?LABEL[m.window]:LABEL[m.window]+" · "+m.filter)),el("td","n",int(t.requests)),el("td","n",int(t.tokIn)+" / "+int(t.tokOut)),el("td","n",usd(t.list)),el("td","n",usd(t.actual)),el("td","n",t.pointsKnown?pts(t.points):"—"));tbody.appendChild(tr);
  ...
  var loadUsage=function(){if(!scrim.isConnected)return;err.textContent="";note.textContent="";a.getCoworkerUsage(agentId,m.window).then(function(r){
      ...
      if(r.error){m.usage=null;var served=!notServed(r.error);if(served)err.textContent="The server could not be asked. "+String(r.error);
        ...
        return (a.getCoworkerSpend?a.getCoworkerSpend(agentId):Promise.resolve(null)).then(function(s){var sp=s&&s.spend;sub.textContent=seatLine(sp)||sub.textContent;var ws=sp&&Array.isArray(sp.windows)?sp.windows:[];var w=null;for(var i=0;i<ws.length;i++)if(ws[i]&&ws[i].window===m.window)w=ws[i];
          m.usage={models:w?[{modelId:"All models",requests:typeof w.requests==="number"?w.requests:0,inputTokens:0,outputTokens:0,costUsd:w.usedUsd,listUsd:w.counterfactualUsd,points:null}]:[]};fillFilter(models().filter(function(id){return id!=="All models"}));paint();
