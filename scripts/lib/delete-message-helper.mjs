...
export const DELETE_MESSAGE_HELPER =
  ...
  + 'if(d&&typeof d.getTranscriptDeletion==="function"){d.getTranscriptDeletion().then(function(r){self.__sandDeleteAvailable=!!(r&&r.available===!0)}).catch(function(){})}'
  + 'var open=null;'
  ...
  + 'self.__sandDeleteMessage=function(id,hint){id=String(id||"");if(!id)return!1;var q=JSON.stringify(id);var row=document.querySelector("[data-entry-id="+q+"]")||document.querySelector(".sand-transcript-row[aria-labelledby*="+JSON.stringify("sand-conversation-entry-"+id+"-")+"]")||document.querySelector(".sand-transcript-row[data-row-key="+q+"]");if(!row)return!1;close();'
  ...
  + 'var s=document.createElement("span");s.textContent="Delete this message?";'
  ...
  + 'if(!d||typeof d.deleteTranscriptEntries!=="function"||!agentId){s.textContent="Couldn’t delete: no route to the server.";ok.remove();return}'
  ...
  + 's.textContent="Couldn’t delete: "+why+".";ok.remove()'
  + '}).catch(function(e){s.textContent="Couldn’t delete: "+(e&&e.message?e.message:"the server did not answer")+".";ok.remove()})});'
