// Auto-review for one coworker: a summary on the settings pane, and a modal
// that matches Standing rules (tabs, filter, add/delete) plus inherit / on / off.
//
// The server stores a tri-state per field (null = inherit from global). The
// pane only says what is in force; editing happens in the modal.

export const AGENT_AUTOREVIEW_HELPER = String.raw`;(()=>{try{
var CSS=".sand-lp-ar{margin:14px 0 0;padding:10px 14px;border:1px solid var(--sand-border-default,rgba(127,127,127,.28));border-radius:10px;display:flex;align-items:center;gap:10px}"
+".sand-lp-ar h4{margin:0;font:600 13px system-ui,-apple-system,sans-serif}"
+".sand-lp-ar .lp-ar-sum{flex:1;min-width:0;margin:0;font:400 12px system-ui,sans-serif;opacity:.7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}"
...
+".sand-ar-sheet{width:min(520px,100%);max-height:calc(100vh - 48px);overflow:auto;box-sizing:border-box;border-radius:16px;padding:20px;border:1px solid var(--sand-border-default,rgba(128,128,128,.24));background:var(--sand-bg-elevated,Canvas);color:var(--sand-text-primary,CanvasText);box-shadow:0 24px 60px rgba(0,0,0,.35);font:13px/1.45 system-ui,-apple-system,sans-serif;display:flex;flex-direction:column;gap:12px;outline:none}"
...
+".sand-ar-sub{margin:0;font-size:12px;color:var(--sand-text-secondary,#5a5a5a)}"
+".sand-ar-seg{display:inline-flex;border:1px solid var(--sand-border-default,rgba(128,128,128,.3));border-radius:8px;overflow:hidden}"
+".sand-ar-seg button{font:inherit;font-size:12px;color:inherit;background:transparent;border:0;padding:5px 10px;cursor:pointer}"
+".sand-ar-seg button[aria-pressed=true]{background:var(--sand-fill-neutral-subtle,rgba(128,128,128,.18));font-weight:600}"
+".sand-ar-tabs{display:flex;gap:8px}"
+".sand-ar-tabs button{font:inherit;font-size:12px;color:inherit;background:transparent;border:1px solid var(--sand-border-default,rgba(128,128,128,.3));border-radius:8px;padding:5px 10px;cursor:pointer}"
+".sand-ar-tabs button[aria-selected=true]{background:var(--sand-fill-neutral-subtle,rgba(128,128,128,.18));font-weight:600}"
+".sand-ar-filter,.sand-ar-add input{font:inherit;font-size:13px;color:inherit;background:transparent;border:1px solid var(--sand-border-default,rgba(128,128,128,.3));border-radius:8px;padding:0 10px;height:34px;width:100%;box-sizing:border-box}"
+".sand-ar-list{overflow:auto;min-width:0}"
+".sand-ar-row{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 0;min-width:0}"
...
+".sand-ar-row button,.sand-ar-add button{font:inherit;font-size:12px;color:inherit;background:transparent;border:1px solid var(--sand-border-default,rgba(128,128,128,.3));border-radius:8px;padding:5px 10px;cursor:pointer;flex:none}"
+".sand-ar-add{display:flex;gap:8px;align-items:center}"
+".sand-ar-add input{flex:1;min-width:0}"
...
var summaryOf=function(mode,allow,block){var n=allow.length+block.length;var rules=n===0?"no rules of its own":n+(n===1?" rule":" rules")+" — "+allow.length+" allow, "+block.length+" block";if(mode==="on")return"On. "+rules+".";if(mode==="off")return"Off. "+rules+".";return"Inherits global. "+rules+"."};
...
var open=function(agentId){agentId=agentId||currentAgent();var a=window.desktop&&window.desktop.agent;if(!a||!a.getAgentAutoReview||!agentId)return null;
  close();style();
  var m={agentId:agentId,mode:"inherit",tab:"allow",allow:[],block:[],filter:"",busy:false};
  ...
  var add=el("div","sand-ar-add");var draft=el("input");draft.type="text";draft.setAttribute("aria-label","Rule to add");
  var addBtn=el("button",null,"Allow");addBtn.type="button";
  var addRule=function(){var p=draft.value.trim();if(!p||m.busy)return;var bucket=m.tab==="allow"?m.allow:m.block;if(bucket.indexOf(p)>=0){draft.value="";return}bucket.push(p);draft.value="";paintTabs();paintList();persist()};
  draft.addEventListener("keydown",function(e){if(e.key==="Enter"){e.preventDefault();addRule()}});
  addBtn.addEventListener("click",addRule);add.append(draft,addBtn);
  ...
  var paintList=function(){list.textContent="";var rules=m.tab==="allow"?m.allow:m.block;var needle=m.filter.trim().toLowerCase();var shown=needle?rules.filter(function(p){return p.toLowerCase().indexOf(needle)>=0}):rules;list.style.height="min(38vh, "+Math.max(96,rules.length*38)+"px)";
  ...
  var load=function(){return a.getAgentAutoReview(agentId).then(function(r){
    ...
    var eff=r.effective||{};var by=(eff&&eff.decidedBy)||{};
    var w=function(f){return by[f]==="coworker"?"this coworker":by[f]==="global"?"global":"default"};
    sub.textContent="Effective: "+(eff.enabled?"on":"off")+" · enabled from "+w("enabled")+", allow from "+w("allowInstructions")+", block from "+w("blockInstructions")+".";
    paintMode();paintTabs();paintList();refreshPane()}).catch(function(e){err.textContent=String(e&&e.message||e)})};
  var refreshPane=function(){var box=document.querySelector(".sand-lp-ar");if(box&&box.parts&&box.parts.sum)box.parts.sum.textContent=summaryOf(m.mode,m.allow,m.block)};
  sheet.append(top,sub,seg,tabs,filter,list,add,err);scrim.appendChild(sheet);document.body.appendChild(scrim);
  onKey=function(e){if(e.key==="Escape"){e.preventDefault();close()}};document.addEventListener("keydown",onKey);
  scrim.addEventListener("mousedown",function(e){if(e.target===scrim)close()});
  m.el=scrim;m.parts={sub:sub,seg:seg,tabs:tabs,list:list,draft:draft,addBtn:addBtn,err:err,filter:filter};modal=m;
...
var mount=function(pane){var agentId=currentAgent();if(!agentId)return;
  ...
  var h=el("h4",null,"Auto-review");var sum=el("p","lp-ar-sum","Reading…");
  ...
  box.append(h,sum,btn);box.parts={sum:sum,open:btn};pane.appendChild(box);
  var load=function(){if(!box.isConnected)return;a.getAgentAutoReview(agentId).then(function(r){
    ...
    if(r.error){sum.textContent=String(r.error);return}
    var row=r.row||null;
    var mode=row&&typeof row.enabled==="boolean"?(row.enabled?"on":"off"):"inherit";
    sum.textContent=summaryOf(mode,rowsOf(row&&row.allowInstructions),rowsOf(row&&row.blockInstructions));
  }).catch(function(e){sum.textContent=String(e&&e.message||e)})};
...
window.__sandAutoReview={open:open,close:close,current:function(){return modal},summary:summaryOf,rowsOf:rowsOf};
