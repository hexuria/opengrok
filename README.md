...
`bootstrap` prefers the hash-checked 0.18.0 DMG at `research-archives/original/0.18.0/macos-arm64/Grok_Bot_0.18.0.dmg` over the Cursor CDN, then extracts what the build needs. Set `GROK_BOT_018_APP` to a 0.18.0 `Grok Bot.app` to skip the DMG. `package` typechecks, tests, compiles the Vite UI, signs and verifies. Output is `dist/Open Grok.app`.
